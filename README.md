...
```sh
ansible-navigator run playbooks/disable_schedules.yml --eei registry.redhat.io/ansible-automation-platform-24/ee-supported-rhel9 -i localhost, -e "dev"
```

* To disable all active schedules. IDs of the scheduled jobs will be kept in a file in the playbook directory.

```sh
ansible-navigator run playbooks/disable_schedules.yml --ask-vault-password -i localhost, -e "dev"
```

* To re-enable all schedules disabled in the previous step

```sh
ansible-navigator run playbooks/enable_schedules.yml --ask-vault-password -i localhost, -e "dev"
```
